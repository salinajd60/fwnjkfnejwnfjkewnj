...
// const SOLANA_MINT_PUBKEY = loadKeypairFromFile("./keys/devnet/solana_mint.json").publicKey;
const SOLANA_MINT_PUBKEY = new PublicKey(
  "Gm9MubdxyeHqyrCADG1rKzSQWeNC53fcayB3rg1fT9Uv"
);

const RECIPIENT = loadKeypairFromFile("./keys/devnet/recipient.json");
const BRIDGE_AMOUNT = new anchor.BN(1_000000000);
const ADDITIONAL_SOL_GAS = new anchor.BN(5000000);
...
async function main() {
  ...
  const [tokenInfo, tokenInfoBump] = PublicKey.findProgramAddressSync(
    [Buffer.from("token_info"), bridgeHandler.toBuffer(), SOLANA_MINT_PUBKEY.toBuffer()],
    program.programId
  );
  ...
  const [targetMint, targetMintBump] = PublicKey.findProgramAddressSync(
    [Buffer.from("mint"), bridgeHandler.toBuffer(), SOLANA_MINT_PUBKEY.toBuffer()],
    program.programId
  );
  ...
  //   null,
  //   9,
  //   loadKeypairFromFile("./keys/devnet/solana_mint.json")
  // );
  ...
  // // console.log(`signer vault is: ${signerVault.toString()}`);
  // // console.log(`mint is: ${SOLANA_MINT.publicKey.toString()}`);
